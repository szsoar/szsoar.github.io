...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let currentPage = 1;
  const totalPages = Math.ceil(aeExpressionsArticles.length / itemsPerPage);
  ...
  function renderArticles() {
    ...
    const endIndex = Math.min(startIndex + itemsPerPage, aeExpressionsArticles.length);

    // 创建文章卡片
    for (let i = startIndex; i < endIndex; i++) {
      const article = aeExpressionsArticles[i];
