...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let currentPage = 1;
  const totalPages = Math.ceil(aePluginsArticles.length / itemsPerPage);
  ...
  function renderArticles() {
    ...
    const endIndex = Math.min(startIndex + itemsPerPage, aePluginsArticles.length);

    // 创建文章卡片
    for (let i = startIndex; i < endIndex; i++) {
      const article = aePluginsArticles[i];
