...
document.addEventListener('DOMContentLoaded', function () {
  ...
  let currentPage = 1;
  const totalPages = Math.ceil(aeScriptingArticles.length / itemsPerPage);
  ...
  function renderArticles() {
    ...
    const endIndex = Math.min(startIndex + itemsPerPage, aeScriptingArticles.length);

    // 创建文章卡片
    for (let i = startIndex; i < endIndex; i++) {
      const article = aeScriptingArticles[i];
